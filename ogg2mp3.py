...
def get_matched_fields(note, find_this_re = defaul_search_re): #note can be note_dict or nid
    fields_list = []
...
def filter_matched_note(note, find_this_re = defaul_search_re): #note can be note_dict or nid
    note_schema               = {}
...
def convert_file(note):
    ...
    for s, d in zip(source_files, destination_files):
        ...
        conversion_command = [ffmpeg_path,'-hide_banner','-loglevel','error','-i', source_file, destination_file]
        try:
...
def rm_file(note_schema, undo = False):
    file_names = note_schema['file_names'] if not undo else note_schema['new_file_names'] 
    for f in file_names:
...
def update_field(note_schema, undo = False):
    nid = note_schema['nid']
    new_fields = note_schema['new_fields'] if not undo else note_schema['fields'] # undo is for roll-back
    for f_name, f_text in new_fields.items():
        invoke('updateNoteFields', note = {"id": nid, "fields": {f_name: f_text}})
        print('note {}\'s fields has been updated. Field of {} has become {}.'.format(nid, f_name, f_text))
